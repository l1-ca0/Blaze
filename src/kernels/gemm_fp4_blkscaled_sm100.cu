...
void fp4_blkscaled_mma_consumer(
    ...
    for (int k_tile = 0; k_tile < num_k_tiles; k_tile++) {
        ...
        for (int ki = 0; ki < Config::NUM_K_ITERS; ki++) {
            // A is [TILE_M, TILE_K/2] in SMEM, row stride = TILE_K/2.
            // Each K-atom advances by K_PER_MMA/2 columns (packed bytes).
            uint32_t smem_a = smem_a_base + ki * (Config::K_PER_MMA / 2);
            // B is [TILE_K, TILE_N/2] in SMEM, row stride = TILE_N/2.
            // Each K-atom advances by K_PER_MMA/2 rows × TILE_N/2 bytes/row.
            uint32_t smem_b = smem_b_base + ki * (Config::K_PER_MMA / 2) * (Config::TILE_N / 2);
